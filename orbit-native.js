(function(){
  ...
  /**
   * Native share: write blob to CACHE, then Share.share({files:[uri]}) first.
   * Never treat text-only as success for invoice share.
   */
  window.__orbitNativeShare = async function(opts){
    ...
    if(hasCap() && Share && Share.share && Filesystem && Filesystem.writeFile && blob){
      try{
        ...
        // CACHE is app-private and FileProvider-safe (no storage permission).
        var attempts = [
        ...
        for(var u = 0; u < uris.length; u++){
          ...
          try{
            ...
            try{ console.warn("orbit share files-only", eFilesOnly); }catch(e2){}
          }
          // 2) files + short text
          try{
            await Share.share({
        ...
        if(lastErr) try{ console.warn("orbit share write", lastErr); }catch(e){}
      }catch(eCap){
        try{ console.warn("orbit native share", eCap); }catch(e2){}
      }
    }

    // Web Share API fallback (PWA / browser)
  ...
  // Watch for successful sale messages and trigger share when needed
  function orbitWatchSaleSuccessForShare(){
    try{
      var obs = new MutationObserver(function(){
        try{
          var el = document.querySelector(".invoiceMsg.ok, .sale-success, [data-invoice-ready]");
          if(el && !el.dataset.orbitShareTried){
            el.dataset.orbitShareTried = "1";
            if(typeof window.prepareAndOpenInvoiceShare === "function"){
              setTimeout(function(){ try{ window.prepareAndOpenInvoiceShare("png"); }catch(e){} }, 400);
            }
          }
        }catch(e){}
      });
